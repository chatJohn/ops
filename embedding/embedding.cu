...
__global__ void embedding_f32_kernel(const int *idx, float *weight,
    ...
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int global_tx = bx * blockDim.x + tx;
...
__global__ void embedding_f32x4_pack_kernel(const int *idx, float *weight,
    ...
    int offset = idx[bx] * emb_size;
    LDST128BITS(output[bx * emb_size + tx * 4]) = LDST128BITS(weight[offset + tx * 4]);
}

__global__ void embedding_f16_kernel(const int *idx, half *weight,
...
#define TORCH_BINDING_EMBEDDING(packed_type, th_type, element_type, n_elements)
    void embedding_##packed_type(torch::Tensor a, torch::Tensor weight, torch::Tensor o){
        ...
        dim3 grid(N); // each block handles one embedding
        embedding_##packed_type##_kernel<<<grid, block>>>(reinterpret_cast<int *>(a.data_ptr())
...
TORCH_BINDING_EMBEDDING(f16, torch::kFloat16, half, 1)
TORCH_BINDING_EMBEDDING(f16x8, torch::kFloat16, half, 8)
TORCH_BINDING_EMBEDDING(f16x8_pack, torch::kFloat16, half, 8)

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){ // if use `JIT` to compile, the `name` will be showed in `xx.py` file
